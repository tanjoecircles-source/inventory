...
class StorePurchasingController extends Controller
{
    public function list(Request $request)
    {
        ...
        if($request->ajax()){
            $view = view('web.admin.store_purchasing.paginate', $data)->render();
            return response()->json(['html' => $view]);
        }
        return view('web.admin.store_purchasing.list', $data);
    }

    public function combo(Request $request){
        $data = $request->all();
        $keyword = empty($data['search']) ? "" : $data['search'];
        $contents = DB::table('store_purchasing')
    ...
    public function add()
    {
        ...
        return view('web.admin.store_purchasing.add', $data);
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($insert){
            DB::commit();
            return redirect('store-purchasing-detail/'.$insert->id)->with('success','Data has been created');
        }else{
            DB::rollback();
    ...
    {
        $data = ['pur_id' => $id];
        return view('web.admin.store_purchasing.item_add', $data);
    }
    ...
    public function itemCreate(Request $request)
    {
        ...
        DB::beginTransaction();
        $insert = StorePurchasingItem::create([
        ...
        if ($insert){
            DB::commit();
            return redirect('store-purchasing-detail/'.$data['pur_id'])->with('success','Data has been created');
        }else{
            DB::rollback();
            return redirect()->back()->with('danger', 'Data failed to create, try again later');
        }
    ...
    public function itemDetail($id)
    {
        ...
        $detail = DB::table('store_purchasing_items AS s')
            ...
            ->first();
        $data = ['detail' => $detail];
        return view('web.admin.store_purchasing.item_detail', $data);
    }   

    public function itemDelete($id)
    {
        $pur_id = $_GET['pur'];
        $data = StorePurchasingItem::find($id);
        if (is_null($data)){
            return redirect('store-purchasing-detail/'.$pur_id)->with('danger','Something Wrong, data not found.');
        }elseif (!$data->delete()){
            return redirect('store-purchasing-detail/'.$pur_id)->with('danger','Something Wrong, Data failed to delete.');
        }else{
            return redirect('store-purchasing-detail/'.$pur_id)->with('success','Data has been deleted.');
        }
    }

    public function detail($id)
    {
        $contents = DB::table('store_purchasing_items')
        ...
        $detail = DB::table('store_purchasing AS s')
            ->leftJoin('vendor AS c', 'c.id', '=', 's.pur_vendor')
            ->select('s.*',
        ...
        $data = [
            'store_purchasing' => $detail,
            'pur_sub_total' => $itm_total,
            'pur_total' => (INT)$itm_total - (INT)$detail->pur_discount,
            'contents' => $contents
        ];

        return view('web.admin.store_purchasing.detail', $data);
    }

    public function edit($id)
    {
        $detail = StorePurchasing::where(['id' => $id])->first();
        $data = [
            'detail' => $detail,
            'vendor' => Vendor::all(),
            'pur_date' => date('d-m-Y', strtotime($detail->pur_date))
        ];
        return view('web.admin.store_purchasing.edit', $data);
    }
    ...
    public function publish($id)
    {
        ...
        if ($update){
            DB::commit();
            return redirect('store-purchasing-list')->with('success','Data has been updated');
        }else{
            DB::rollback();
            return redirect()->back()->with('danger', 'Data failed to update, try again later');
        }
    ...
    public function payment($id)
    {
        ...
        return view('web.admin.store_purchasing.payment', $data);
    }
    ...
    public function set($id)
    {
        ...
        return view('web.admin.store_purchasing.set', $data);
    }
    ...
    {
        $data = StorePurchasing::find($id);
        if (is_null($data)){
            return redirect('store-purchasing-detail/'.$id)->with('danger','Something Wrong, data not found.');
        }elseif (!$data->delete()){
            return redirect('store-purchasing-detail/'.$id)->with('danger','Something Wrong, Data failed to delete.');
        }else{
            return redirect('store-purchasing-list')->with('success','Data has been deleted.');
        }
    }
}
