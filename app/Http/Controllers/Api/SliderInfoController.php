...
class SliderInfoController extends Controller
{
    public function list()
    {
        ...
        $list = DB::table('slider_info as si')
            ->select('si.*')
            ->get();
    ...
    public function create(Request $request)
    {
        ...
        $exist = SliderInfo::where('name', $param['name'])->count();
        if($exist > 0) return response(['status' => false, 'message' => 'Data sudah terdaftar'], 400);

        $photo = ($request->file('photo')) ? $request->file('photo')->store('slider', 'public') : null;

        $params = array_merge(['photo' => $photo], $param);
        $result = SliderInfo::create($params);
        if($result)
    ...
    public function update(Request $request)
    {
        ...
        $old = SliderInfo::where('id', $param['id'])->first();
        if($request->file('photo')) {
            File::delete(storage_path('app/public/'.$old['photo']));
            $photo = $request->file('photo')->store('slider', 'public');
        } else { 
            $photo = $old['photo'];
        }
        
        $result = SliderInfo::where('id',$param['id'])->update(['name'=>$param['name'], 'photo'=>$photo, 'description' => $param['description']]);
        if($result)
            return response(['status' => true, 'message' => 'berhasil menyimpan data'], 200);
        else
            return response(['status' => false, 'message' => 'gagal menyimpan data'], 400); 
    ...
    public function delete(Request $request)
    {
        ...
        $result=SliderInfo::where('id', request()->only('id'))->delete();
        if($result)
