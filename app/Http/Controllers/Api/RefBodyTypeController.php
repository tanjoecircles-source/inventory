...
class RefBodyTypeController extends Controller
{
    public function list(Request $request)
    {
        ...
        $list = DB::table('ref_body_type as bt')
            ->select('bt.*')
            ->get();
    ...
    public function update(Request $request)
    {
        ...
        $result = RefBodyType::where('id',$param['id'])->update(['name'=>$param['name']]);
        if($result)
            return response(['status' => true, 'message' => 'berhasil menyimpan data'], 200);
        else
            return response(['status' => false, 'message' => 'gagal menyimpan data'], 400); 
    ...
    public function delete(Request $request)
    {
        ...
        $result=RefBodyType::where('id',$param['id'])->delete();
