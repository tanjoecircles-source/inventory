...
<div class="card mb-3" id="card-{{$content->id_produk}}">
    <div class="card-body p-2 row" style="line-height:20px">
        <div class="col-4 pr-1">
            <a href="{{ url('etalase-detail/'.$content->id_etalase) }}">
        ...
        <div class="col-8 pl-1">
            ...
            <p class="text-default p-0 mb-0 d-block mt-1 fs-14 h-40">{{$content->judul}}</p>
            </a>
            <div class="d-flex flex-row">
                ...
                <div class="">
                    @if($content->status_etalase == 'true')
                    <a href="#" id="rmv_etalase_{{$content->id_produk}}" onclick="removeEtalase('{{$content->id_produk}}')" class="btn btn-white btn-block m-0 ml-auto fs-12 mt-1" style="padding:2px 6px 4px;"><span id="label-rmv-{{$content->id_produk}}"><i class="fe fe-trash"></i> Hapus</span></a>
                    @else
                    <span class="badge badge-success mt-2 pull-right">Transaksi</span>
                    @endif
                </div>
            </div>   
        </div>
...
function removeEtalase(id){
    ...
    .done(function(data){
        if(data.status == 0){
            $('#card-'+data.id).remove();
            $('#etalase-count').replaceWith('<h6>Etalase ('+data.count+')</h6>');
        }
    })
