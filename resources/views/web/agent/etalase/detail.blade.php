<x-layouts.app>
    <x-header-white-3column>
        @slot('back')
        <x-back backstyle="text-dark" urlback="{{url('etalase')}}"></x-back>
        @endslot
        @slot('notif')
        <x-share sharestyle="text-dark" shareurl="{{url('share-catalogue-product?id='.$id_produk)}}"></x-share>
        @endslot
    </x-header-white-3column>
    <style>
            ...
            <div class="col-sm-12 col-md-12 col-lg-8 mx-auto px-0">
                ...
                <div class="card text-center no-border shadow-none custom-square mb-7">
                    <div class="card-body p-2">
                        @if($visit_submited == 0)
                            <a href="{{url('visitasi/'.$id_etalase)}}" class="btn btn-primary btn-lg btn-block"><i class="fe fe-calendar"></i> Ajukan Visitasi</a>
                        @else
                            <button class="btn btn-gray btn-lg btn-block"><i class="fe fe-refresh-ccw"></i> Dalam Proses Transaksi</button>
                        @endif
                        @if($info['status_etalase'] == 'true')
...
function removeEtalase(id){
    $.ajax({
        ...
        type:'post',
        beforeSend: function(){
            setTimeout($('#label-rmv-etalase').replaceWith('<i class="fa fa-circle-o-notch fa-spin" style="font-size:12px"></i> proses...'), 10000);
        }
    })
    .done(function(data){
        if(data.status == 0){
            window.location.href = "{{url('etalase')}}";
        }
    })
