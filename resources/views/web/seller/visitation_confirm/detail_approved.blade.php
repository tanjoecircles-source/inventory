        ...
        <div class="col-sm-12 col-md-12 col-lg-8 mx-auto">
            ...
            <div class="tab-content" id="pills-tabContent">
                <div class="tab-pane fade show active" id="pills-list-agent" role="tabpanel" aria-labelledby="pills-list-agent-tab">
                    ...
                    @endif
                    <div id="content-data">
                        @if(empty($listApproved->count()))
                            <h6 class="m-4 text-center">No matching records found</h6>
                        @endif
                        @include('web.seller.visitation_confirm.paginate_detail_approved')
                    </div>
...
<!-- Modal -->
<div class="modal fade" id="qrcode-dialog" role="dialog" >
    <div class="modal-dialog">
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title text-center w-100 font-weight-bolder">QR Code Unit</h4>
...
<script>
    ...
    $(document).on('click', '.btn-qrcode-unit', function(e){
        e.preventDefault();
        e.stopImmediatePropagation();
        var visitId = $(this).data('visitid');
        $('#qrcode-dialog .modal-body').html('<center>Harap Menunggu...</center>');
        $.ajax({
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            url: "{{ url('visitation-confirm/qrcode-visitation') }}",
            type: 'POST',
            dataType: 'html',
            data: {
                id: visitId
            ...
            success: function(r){
                $('#qrcode-dialog .modal-body').html(r);
            },
            error: function(e){
                $('#qrcode-dialog .modal-body').html('<center>Terjadi kesalahan, coba keluar qr code penjualan dan buka kembali</center>');
            }
        });
        $('#qrcode-dialog').modal('show');
    });
</script>
</x-layouts.app>