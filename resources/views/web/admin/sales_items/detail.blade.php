...
<x-header-white-3column>
    @slot('back')
    <x-back backstyle="text-dark" urlback="{{url('sales-detail/'.$detail->itm_inv_id)}}"></x-back>
    @endslot
    @slot('notif')
        ...
        <div class="col-sm-12 col-md-12 col-lg-8 mx-auto">
            <div class="card no-border shadow-none custom-square mt-4 mb-3">
                <div class="card-body px-2 py-4">
                    ...
                    <div class="form-group">
                        ...
                        <h6 class="px-2 m-0 font-weight-bold">{{$detail->product_price}}</h6>
                    </div>
                    <div class="form-group">
                        <p class="px-2 mb-2">Qty</p>
                        <h6 class="px-2 m-0 font-weight-bold">{{$detail->itm_qty}}</h6>
                    </div>
                    <div class="form-group">
                        <p class="px-2 mb-2">Total Invoice</p>
                        <h6 class="px-2 m-0 font-weight-bold">{{$detail->itm_total}}</h6>
                    </div>
                </div>
            </div>
            <div class="card text-center no-border shadow-none custom-square mb-7">
                <div class="card-body p-0">
                    @if($detail->type == '1')
                    <a href="{{(url('product-print/'.$detail->itm_product))}}" class="d-flex p-4 border-bottom">
                        <i class="fe fe-printer fs-16 mr-2"></i>
                        <h6 class="mb-1 font-weight-semibold">Cetak Info Kopi</h6>
                    </a>
                    @endif
                    <a href="{{(url('sales-item-delete/'.$detail->id.'?inv='.$detail->itm_inv_id))}}" data-title="{{$detail->product_name}}" class="d-flex p-4 border-bottom btn-confirm">
