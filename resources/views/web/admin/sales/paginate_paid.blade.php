...
@foreach ($contents as $content)
    <a href="{{ url('sales-detail/'.$content->id.'&cust='.$content->csurl.'&date='.$content->dturl.'&author='.$content->authurl) }}" class="card mb-2 animate__animated animate__fadeIn">
        <div class="card-body px-2 py-2">
            <div class="d-flex title-bar">
                <div class="mr-auto text-left">
                    <p class="mb-1">#{{$content->inv_code}}</p>
                    <p class="mb-1 text-muted">{{$content->inv_date}}</p>
                    <span class="badge badge-pill badge-{{$content->inv_status_color}} ml-auto mr-0 py-1">{{$content->inv_status_label}}</span>
                </div>
