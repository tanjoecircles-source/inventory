...
<x-header-white-3column>
    @slot('back')
    <x-back backstyle="text-dark" urlback="{{url('store-recap-detail/'.$profit->id)}}"></x-back>
    @endslot
    @slot('notif')
...
<div class="container">
    <div class="row">
        <div class="col-sm-12 col-md-12 col-lg-8 mx-auto">
            <form id="store-recap-pay-form" name="store-recap-pay-form" action="{{url('store-recap-share-create/'.$profit->id)}}" method="POST" enctype="multipart/form-data" >
            @csrf
            <div class="card no-border shadow-none custom-square mt-4 mb-3">
                <div class="card-body px-2 py-4">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('jenis_potongan_id') is-invalid @enderror" name="jenis_potongan_id" id="jenis_potongan_id" placeholder="Pilih Jenis Potongan">
                            @foreach($spending as $value)
                                @if(old('jenis_potongan_id') == $value->id)
                                    <option value="{{ $value->id }}" selected>{{ $value->name }}</option>
                                @endif
                            @endforeach
                        </select>
                        @error('jenis_potongan_id') <div class="text-primary fs-11">{{ $message }}</div> @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <label class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="share_spending">
                            <span class="custom-control-label">Bagi Rata</span>
                        </label>
...
$(document).ready(function () {
    ...
    $('#jenis_potongan_id').select2({
        "ajax" : {
            "url" : "{{url('store-recap-spending-combo')}}",
            "type" : "POST",
            "dataType" : "json",
            "data": function (params) {
                var query = {
    ...
    $("#jenis_potongan_id").on('change', function(e) {
        let sub_total = $("#sub_total").val();
        let id = $(this).val();
        let url = "{{url('store-recap-spending-json')}}";
        $.ajax({
            type:'GET',
            url:"{{url('store-recap-spending-json')}}",
            data:{'id':id},
            success:function(data){
                $("#potongan").val(data.amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
                total = sub_total.replace(/\./g,"") - data.amount;
                $("#total").val(total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
            }
        }); 
    });    

    $('#share_spending').change(function() {
        let total = $("#sub_total").val();
        let potongan = $("#potongan").val();
        if ($(this).is(':checked')) {
            potongan = potongan.replace(/\./g,"") / 4
            $("#potongan").val(potongan.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
            total = total.replace(/\./g,"") - potongan
            $("#total").val(total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
        } else {
            potongan = potongan.replace(/\./g,"") * 4
            $("#potongan").val(potongan.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
