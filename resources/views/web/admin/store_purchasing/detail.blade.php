        ...
        <div class="col-sm-12 col-md-12 col-lg-8 mx-auto">
            <div class="card no-border shadow-none custom-square mt-4 mb-3">
                <div class="card-body px-2 py-4">
                    <div class="row">
                        <div class="col-7">
                            <div class="form-group">
                                ...
                                <h6 class="px-2 m-0 font-weight-bold">{{$store_purchasing->pur_category}}</h6>
                            </div>
                            <div class="form-group">
                                <p class="px-2 mb-2">Kode Invoice</p>
                                <h6 class="px-2 m-0 font-weight-bold">{{$store_purchasing->pur_code}}</h6>
                            </div>
                        </div>
                        <div class="col-5">
                            <div class="form-group">
                                <p class="px-2 mb-2">Tanggal Invoice</p>
                                <h6 class="px-2 m-0 font-weight-bold">{{date('d M Y', strtotime($store_purchasing->pur_date))}}</h6>
                            </div>
                            <div class="form-group">
                                <p class="px-2 mb-2">Pelanggan</p>
                                <h6 class="px-2 m-0 font-weight-bold">{{$store_purchasing->vendor_name}}</h6>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                    <a href="{{(url('store-purchasing-edit/'.$store_purchasing->id))}}" class="btn btn-outline-dark btn-sm mr-1 px-2 py-1"><i class="fe fe-edit"></i> Ubah</a>
                    <a href="{{(url('store-purchasing-delete/'.$store_purchasing->id))}}" data-title="{{$store_purchasing->pur_code}}" class="btn btn-outline-danger btn-sm btn-confirm px-2 py-1"><i class="fe fe-trash"></i> Hapus</a>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card text-center no-border shadow-none custom-square mb-2">
                <div class="card-body p-0">
                    ...
                    <a href="{{url('store-purchasing-item-detail/'.$content->id.'?pur='.$content->itm_pur_id)}}" class="">
                ...
                @if($store_purchasing->pur_status == 'Draft')
                <a href="{{url('store-purchasing-item-add/'.$store_purchasing->id)}}" class="card-body p-2 border-top font-weight-semibold"><i class="fe fe-plus-circle fs-16"></i> Tambah Produk</a>
                @endif
            </div>
            <div class="d-flex py-2 px-2 border-bottom">
                <div class="mr-auto text-left">
                ...
                @if($store_purchasing->pur_status == 'Draft' && $store_purchasing->pur_total != '')
                <div class="ml-auto text-right">
                    <a href="{{url('store-purchasing-set/'.$store_purchasing->id)}}"><i class="fe fe-settings fs-18"></i></a>
                </div>
                @endif
            </div>
            <div class="card text-center no-border shadow-none custom-square mb-2">
                <div class="card-body p-2">
                    ...
                    @if(!empty($store_purchasing->pur_discount))
                    <div class="d-flex title-bar py-2">
                        <div class="mr-auto text-left">
                            <p class="mb-1">Discount</p> 
                        </div>
                        <div class="ml-auto text-right text-success">
                            <input type="hidden" id="text_disc" class="form-control form-control-sm" name="pur_discount" value="{{$store_purchasing->pur_discount}}">
                            <p id="lab_disc" class="mb-0">- Rp {{str_replace(",", ".", number_format($store_purchasing->pur_discount))}}</p>
                        </div>
                    </div>
                    @endif
                    @if(!empty($store_purchasing->pur_expedition))
                    <div class="d-flex title-bar py-2">
                        <div class="mr-auto text-left">
                            <p class="mb-1">Pengiriman</p> 
                        </div>
                        <div class="ml-auto text-right text-danger">
                            <input type="hidden" id="pur_expedition" class="form-control form-control-sm" name="pur_expedition" value="{{$store_purchasing->pur_expedition}}">
                            <p id="lab_expedition" class="mb-0">+ Rp {{str_replace(",", ".", number_format($store_purchasing->pur_expedition))}}</p>
                        </div>
                    </div>
                    @endif
                    <div class="d-flex title-bar py-2">
                    ...
                    @if($store_purchasing->pur_status == 'Publish')
                        @if($store_purchasing->pur_status_payment == 'Belum Lunas')
                            <a href="{{url('store-purchasing-drafting/'.$store_purchasing->id)}}" data-title="{{$store_purchasing->pur_code}}" class="btn btn-outline-dark btn-block btn-draft" id="btn-draft" name="btn-draft">Drafting</a>
                        @endif
                    @endif 
                    </form>
                </div>
            ...
            <div class="card text-center no-border shadow-none custom-square mb-2">
                <div class="card-body p-2">
                    <div class="d-flex title-bar py-2">
                        ...
                        <div class="ml-auto text-right">
                            <p class="mb-0">{{$store_purchasing->pur_status_payment}}</p>
                        </div>
                    </div>
                    <div class="d-flex title-bar py-2">
                        <div class="mr-auto text-left">
                        ...
                        <div class="ml-auto text-right">
                            <p class="mb-0">Rp {{str_replace(",", ".", number_format($store_purchasing->pur_payment))}}</p>
                        </div>
                    </div>
                    <div class="d-flex title-bar py-2">
                        <div class="mr-auto text-left">
                        ...
                        <div class="ml-auto text-right">
                            <p class="mb-0">Rp {{str_replace(",", ".", number_format($store_purchasing->must_pay))}}</p>
                        </div>
                    </div>
                    @if($store_purchasing->pur_status_payment == 'Belum Lunas')
                    <a href="{{url('store-purchasing-payment/'.$store_purchasing->id)}}" data-title="{{$store_purchasing->pur_code}}" class="btn btn-dark btn-block btn-payment" id="btn-payment" name="btn-payment"><i class="fe fe-credit-card"></i> Pembayaran</a>
                    @endif
                </div>
            </div>
            @endif
