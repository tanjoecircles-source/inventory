        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-8 mx-auto">
                <div class="d-flex justify-content-end mx-3">
                    <a href="{{url('shift-schedule-add?period_id='.$period_id)}}" id="btn-float" class="shadow-sm">
                ...
                <div class="card no-border shadow-none custom-square mb-0">
                    <div class="card-body px-2 py-3">
                        <div class="card-custom-icon text-center">
                            <h4 class="mb-0 font-weight-bold text-gray">{{$schedules->total()}}</h4>
                            <p class="text-dark">data</p>
                        </div>
                        <h4 class="mb-1 font-weight-bold text-primary">Atur Jadwal Shift</h4>
                        <span class="mb-1 text-muted">Manage Employee Shift Schedule</span>
                    </div>
                </div>
                <div class="card no-border shadow-none custom-square mb-2">
                    <div class="card-body px-2 py-2">
                        <form id="filter-form" action="{{url('shift-schedule-list')}}" method="GET">
                            <div class="form-group mb-2">
                                <label class="form-label font-weight-semibold">Pilih Periode</label>
                                <select class="form-control" name="period_id" id="period_select" onchange="this.form.submit()">
                                    <option value="">-- Semua Periode --</option>
                                    @foreach($periods as $p)
                                        <option value="{{ $p->id }}" {{ $period_id == $p->id ? 'selected' : '' }}>
                                            {{ $p->name }} ({{ $p->month }} - {{ $p->week }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="input-group">
                                ...
                                <input type="text" name="keyword" value="{{ request('keyword') }}" class="form-control br-tl-7 br-bl-7" placeholder="Cari karyawan atau tanggal...">
                ...
                <div class="panel text-center no-border shadow-none custom-square">
                    <div class="panel-body px-0 py-2">
                        <div id="content-data">
                            ...
                            @endif
                            @include('web.admin.shift_schedule.paginate')
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $(window).scroll(function(){
            ...
            if ($(window).scrollTop() >= $(document).height() - $(window).height() - 1){
                page++;
                loadMoreData(page, key, pid);
            }
        });
    
        function loadMoreData(page, key, pid){
            $.ajax({
                url:'?page=' + page + '&keyword=' + key + '&period_id=' + pid,
                type:'get',
                beforeSend: function(){
                    $('.ajax-load').show();
                }
