...
<x-header-white-3column>
    @slot('back')
    <x-back backstyle="text-dark" urlback="{{url('bean-recap-detail/'.$profit->id)}}"></x-back>
    @endslot
    @slot('notif')
...
<div class="container">
    <div class="row">
        <div class="col-sm-12 col-md-12 col-lg-8 mx-auto">
            <form id="bean-recap-pay-form" name="bean-recap-pay-form" action="{{url('bean-recap-share-create/'.$profit->id)}}" method="POST" enctype="multipart/form-data" >
            @csrf
...
$(document).ready(function () {
    ...
    $('#jenis_potongan_id').select2({
        "ajax" : {
            "url" : "{{url('bean-recap-spending-combo')}}",
            "type" : "POST",
            "dataType" : "json",
            "data": function (params) {
                var query = {
    ...
    $("#jenis_potongan_id").on('change', function(e) {
        ...
        let id = $(this).val();
        let url = "{{url('bean-recap-spending-json')}}";
        $.ajax({
            type:'GET',
            url:"{{url('bean-recap-spending-json')}}",
            data:{'id':id},
            success:function(data){
