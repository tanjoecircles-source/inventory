...
<head>
    ...
    <title>Monthly Share Profit Summary - {{ $detail->periode }}</title>
    <style>
...
<body>
    <div class="header">
        <div style="float: left;">
            <h1>Share Profit Summary</h1>
        </div>
        <div style="float: right; text-align: right;">
            <p style="margin: 0; font-size: 10pt;">Period: <strong>{{ $detail->periode }}</strong></p>
        </div>
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <td>Bean Sales (Green & Roasted Bean)</td>
                    <td class="text-right">{{ number_format($bean_recap->profit ?? 0, 0, ',', '.') }}</td>
                </tr>
                <tr>
                    <td>Coffee Shop Operations ({{ ($detail->profit_toko < 0) ? 'Loss' : 'Profit' }})</td>
    ...
    <div class="summary-box">
        <div class="label">NET PROFIT READY FOR DISTRIBUTION (NET PROFIT SHARE)</div>
    ...
    <div class="footer">
        ...
        This report is automatically generated by the Tanjoe Management System. 
        The data shown is final based on the transaction summary for the related period.
        Printed on: {{ date('d/m/Y H:i') }}</p>
    </div>
</body>
</html>
