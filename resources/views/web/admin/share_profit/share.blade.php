...
<x-header-white-3column>
    @slot('back')
    <x-back backstyle="text-dark" urlback="{{url('share-profit-detail/'.$profit->id)}}"></x-back>
    @endslot
    @slot('notif')
...
<div class="container">
    <div class="row">
        <div class="col-sm-12 col-md-12 col-lg-8 mx-auto">
            <form id="share-profit-pay-form" name="share-profit-pay-form" action="{{url('share-profit-share-create/'.$profit->id)}}" method="POST" enctype="multipart/form-data" >
            @csrf
            <div class="card no-border shadow-none custom-square mt-4 mb-3">
                <div class="card-body px-2 py-4">
                    ...
                    <div class="form-group">
                        <label class="form-label">Jumlah Tabungan / Kredit</label>
                        <div class="input-icon mb-3">
                            <span class="input-icon-addon fs-15">Rp</span>
                            <input type="text" class="form-control masked @error('tabungan_credit') is-invalid @enderror" id="tabungan_credit" name="tabungan_credit" value="0" readonly>
                        </div>
                        @error('tabungan_credit')<div class="text-danger">{{ $message }}</div>@enderror
                    </div>
                    <div class="form-group">
...
$(document).ready(function () {
    ...
    $("#employee_id").on('change', function(e) {
        ...
        $.ajax({
            type:'GET',
            url:"{{url('share-profit-emppart')}}",
            data:{
                'id_employee':id_employee,
                'id_profit':id_profit
            },
            success:function(data){
                $("#sub_total").val(data.sub_share.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
                $("#total").val(data.total_share.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
                $("#tabungan_credit").val(data.tabungan_credit.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
            }
        }); 
    });
    ...
    $("#potongan").keyup(function(){
        ...
        tabungan_credit = $("#tabungan_credit").val();
        potongan = $(this).val();
        total = sub_total.replace(/\./g,"") - tabungan_credit.replace(/\./g,"") - potongan.replace(/\./g,"");
